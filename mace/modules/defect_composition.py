...
# lattice is a registered site-median of thermal frames (D21). Its sampling noise is ~0.03 A,
# but the time-average of a finite MD keeps whatever tilt pattern the run was trapped in:
# on the data in hand the Pnma point operations and the sub-cell translations of the 2x2x1
# supercell hold only to 0.24-0.35 A on the raw median (its own subgroup, order 8, to
# 0.07 A). This is the tolerance (A) within which an operation is accepted before the
# lattice is symmetrised over the GROUP the accepted operations generate: about half the
# thermal rms, a quarter of the smallest site separation (2.7 A), so that no non-symmetry
# passes. Registered, absolute; the symmetrisation shift it implies is recorded with the
# lattice.
SITE_TOL = 0.3
...
# Two operations with the same point part are the same group element when their
# translations agree modulo the lattice to this (fractional) tolerance; distinct elements
# differ by a supercell translation, >= 1/4 on the tilings in use.
_OP_TOL = 0.05
# A composed translation at the closure's own accuracy is refined later on the symmetrised
# lattice; the closure is capped at the largest group an orthogonal tiling can have.
_CLOSURE_CAP = 48 * 4096


def compose_symmetries(a: Dict[str, Any], b: Dict[str, Any]) -> Dict[str, Any]:
    """`b` after `a`: x -> R_b (R_a x + t_a) + t_b, translation reduced modulo the lattice."""
...
def _same_symmetry(a: Dict[str, Any], b: Dict[str, Any]) -> bool:
    if a["R"] != b["R"]:
        return False
...
def symmetry_closure(ops: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """The group generated by `ops` under composition. Operations found within a tolerance
    on a noisy lattice need not compose within it; symmetrising over the closed group (not
    the found set) is what makes the result exactly invariant under every element."""
    ...
    while changed:
        ...
        for a in list(group):
            for b in list(group):
                c = compose_symmetries(a, b)
                if not any(_same_symmetry(c, g) for g in group):
                    group.append(c)
                    changed = True
                    if len(group) > _CLOSURE_CAP:
                        raise ValueError("symmetry_closure: the operations do not generate a "
                                         "finite group at this tolerance")
    return group
...
def mean_pristine_lattice(model, pristine_frames: Sequence, r_res: float,
    ...
    (`CELL_ORTHO_TOL`). The median is then symmetrised over the group generated by the
    lattice's own symmetry operations, found on it within `tol` (`symmetry_closure`), until
    exactly invariant. Deterministic and label-free given the frame set, whose fingerprint
    is recorded with the residual statistics.
    """
    import hashlib
    ...
    # Symmetrise over the GROUP generated by the operations found within `tol`, then find
    # the operations again on the symmetrised lattice (their translations refined) and
    # repeat: two rounds reach an exactly invariant lattice on the data in hand.
    median = scaled.clone()
    ops: List[Dict[str, Any]] = []
    rounds = 0
    for _ in range(4):
        rounds += 1
        ops = symmetry_closure(lattice_symmetries(scaled, pri_species, cell_mean, tol))
        symmetrised = symmetrise_sites(scaled, pri_species, cell_mean, ops)
        step = float(((symmetrised - scaled) @ torch.tensor(cell_mean)).norm(dim=-1).max())
        scaled = symmetrised - torch.floor(symmetrised)
        if step < 1e-8:
            break
    total = scaled - median
    total = total - torch.round(total)
    symmetrisation = float((total @ torch.tensor(cell_mean)).norm(dim=-1).max())
...
def pristine_placement(model, class_frame, pristine, factors, perm,
    ...
    # The reference lattice's own cell, taken before `pristine` is rebound to the placed
    # density below (whose cell is the class frame's thermal one).
    reference_cell = (np.asarray(pristine[2], dtype=np.float64)
                      if isinstance(pristine, (tuple, list))
                      else pristine.cell.detach().cpu().numpy())
    ...
    matched = match >= 0
    # The tiled REFERENCE cell in the frame's axis order: the ideal lattice's own cell, not
    # the class frame's thermal one -- the symmetry operations are isometries of this cell
    # (a thermal NPT cell is never exactly orthogonal, and the isometry test would keep only
    # the identity).
    ideal_cell = torch.tensor(reference_cell.reshape(3, 3), dtype=dtype)
    ...
    correspondence = {
        ...
        "symmetries": symmetry_closure(lattice_symmetries(scaled, pri_species,
                                                          ideal_cell.numpy())),
    }
