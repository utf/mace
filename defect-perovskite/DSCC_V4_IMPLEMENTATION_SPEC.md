...
| P3.2 | Arm 2+3 (v4.1 layout): regime B — route {A, B} × coupling {LR-only; LR+U; full; λ=1 fixed} + Φ=0 per route = 10 configurations × 6 seeds; regime-A ablation: full coupling × 2 routes × 6 seeds, excluded from selection; thresholds registered; forecasts §2.10 checked | **blocked** — Arm 1 stopped by the registered rule (C8); launch path ready (`dscc_launch_arm23.sh`, `SEEDS=` list; coupled path smoke-tested on the stripped tree with `h0_state.pt` init) |
| P3.3 | Arm 4: matched-kernel and full-kernel F-SCC comparators; decision (1)–(6) | **blocked** (C8); F-SCC training path smoke-tested on the stripped tree (`--fscc matched`) |
