...
2026-09-06 evening) `DSCC_PLAN_V4_3_AMENDMENT.md` (2026-09-07: C8 ruling — precedence of the C5 gate over route C's stop; Arm 2+3 protocol = the Arm-1 protocol; §9 deferred robustness pass; benchmark engineering item), `DSCC_PLAN_V4_5_AMENDMENT.md` (2026-09-07: SCF warm starts during training, C9), `DSCC_PLAN_V4_4_AMENDMENT.md` (required vs optional electrostatics: `K_LR` required and ladder-validated, Φ = 0 arms references, LR-only the minimal candidate; received 2026-09-08 ~15:35, after the Arm-2+3 campaign), `DSCC_C10_RULINGS.md` (2026-09-08: the user's C10 rulings verbatim) and `DSCC_PLAN_V4_2_AMENDMENT.md` (C5–C7 rulings: Route B′,
...
| C11 | **Shell centre defect (D15, 2026-09-09):** the minimum-image flanking-Pb midpoint sits on the occupied bridging Cl in ≈ 45 % of the charged 79-atom frames (the cell is two octahedra thick along z), so every charged 0–2 Å reading before D15 is an artefact and the far-field gate's "decisive 0–2 Å" shell is EMPTY under a correct centre; the full-RMSE and 2–4 Å readings, the B′ gains and the selection are unchanged (shells from 2 Å out move by < 1 meV/Å). `Trainer.evaluate` now uses the vacancy-side centre (`3a6a05c`), so held-out files exist in TWO conventions: every `held_final.json` before it (all 48 Arm-2+3 runs, and the three F-SCC finals, which run on the b3 tree at `be3c39b`) is minimum-image; nothing evaluated with the new code may be compared to an old file, and b3 is not synced until the F-SCC runs and the Arm 4 reading are done. Needed: re-register the gate's decisive shells (2–4 Å, or 0–4 Å) under the corrected centre; whether to re-read the 48 runs' shells (≈ 1.5 h GPU, no effect on the outcome). | open |
| C12 | **Capacity arm `b_i(h_i)` after D15:** the reading by the registered rule is mixed on the substitute 2–4 Å shell (the registered 0–2 Å shell is not readable); the evidence (in the 5.2–6.4 Å bulk a size-independent base residual at matched d with the head's 1.2–6 × smaller at 159 on the same geometry; both diverging at d ≥ 6.4 Å; the base's systematic dimer error removed by the head; 1.7 × larger 79-atom label forces) says the 79-atom near-field excess is a property of the 79-atom labels and cell, not a head-capacity deficit — except the d < 5.2 Å corner, where the head's residual is equal at both sizes (one trained-on 159 frame per bin), unresolved; the arm is not indicated for the bulk; if it runs, its metric is the paired 79 / 159 flanking-Pb residual at matched d with the d < 5.2 Å bins reported separately. | open |
...
the code; this record uses per-component numbers. The registered `tau_phys` = 3 meV/Å was
applied in the trainer's convention (1.7 per component); if it was registered in MACE's
convention the margin is 5.2 in the trainer's numbers — neither selection changes (LR-only vs
Φ = 0 is 1.8 either way; λ = 1 at +3.4 would become equivalent but is out on
`localisation_stable`); the user is asked which convention the registration meant. The shape
margin is an energy-slope quantity and is not affected.
...
lines launched. *v4.4 (received ~15:35):* `K_LR` is required physics "validated by the
tiling-ladder gate (§8)", so the P4.1 model-only tiling ladder runs on the LR-only candidate
(`dscc_arm23_B_A_lr_only_s0`, `~/runs/dscc/ladder_lr_only_s0.json`) as its validation; v4.4
also records that force-only selection cannot resolve the image term — its absence from the
admissible tests is a data limitation, not evidence against it — and that Route B′ and its
scale are optional under the equivalence rule, "off" a legitimate selected value.

*C10 addendum (~15:50, applied).* The B′ ladder gate is re-read as a model property: the
`q0` compensation cloud extends 8–11 Å (`R_c`), so on the ladder cells (11.2 / 16.8 / 22.4 Å,
all below ~3 R_c) the `1/L` coefficient cannot reach Madelung — reported (`ladder_rel_err`),
not gated; B′ is judged on forces; no cross-size energy claim includes `E_SF` until a ladder
with L ≫ R_c exists (the sparse path, P4.2); the double-screening caveat (the pattern's
compensation and `eps_inf` both screening the carrier) is recorded. The four checks the
addendum makes this "subject to" did not reach the record. The B′ campaign is trimmed to
LR-only and LR + U (12 runs; `arm23bp_queue_b3.txt` truncated in place at 15:41, wave 1
unchanged, the driver restarted for 15 runs). The far-field gate is now the B′ gain over the
same-coupling Route A arm on the full per-atom RMSE and on the 0–2, 2–4 shells (decisive) and
the 4–6, 6–8 and pooled 4–8 Å shells (reported), each beyond the Φ = 0 seed spread of that
quantity (`gates()["bprime_gains"]`, `bprime_gain_beyond_noise`); the shells come from the
held-out evaluation, which mixes in the fold's 159-atom frames at < 2 % of the atoms.
`tau_phys` stays in the trainer's convention. The LR-only tiling ladder (v4.4's `K_LR`
validation) is running on the CPU (`dscc_ladder.py --device cuda` fails on a device mismatch,
to fix) and will be read as a model property under the same L ≫ R_c caveat.

*The four checks (user ~16:00; `scratchpad/bp_ladder_checks.py`, `bp_ladder_checks2.py`;
`~/runs/dscc/bp_ladder_checks*.json`; the Arm-2+3 Φ = 0 s0 `H0`, cubic CsPbCl3 tilings
2–6 = 11.2–33.6 Å, the gate's fixed unit carrier on a flanking Pb).* (1) Pristine tiled cells:
`q0` is exactly periodic within every cell (per-species std ≤ 2e-15, sum 4e-14) and the unit
carrier's cross term with a FIXED pattern (the stored per-species `q0`) is −4.4775 meV on every
cell to four digits — the Ewald background is consistent across sizes. The pristine cells'
own per-species `q0` does drift with the supercell (Pb 0.898 / 0.657 / 0.716 / 0.691 / 0.700 e
at 11.2 / 16.8 / 22.4 / 28.0 / 33.6 Å): the Γ-point sampling of the fill, converging by ≈ 22 Å;
its cross term is meV-scale and does not move the ladder. (2) Consecutive-pair slopes of the
model's `q0` pattern against Madelung: 2/3: 41 %, 3/4: 78 %, 4/5: 2 %, 5/6: 97 % (full fit
49 %) — no shrinkage with L. The point-charge locally neutral pattern (the missing ion's charge
at the vacancy point) reads 9.8 / 0.2 / 0.0 / 0.0 % on the same cells, and fixed Gaussian
clouds shrink as expected (r_gyr 7.5 Å: 43 → 32 → 24 → 15 %), so the ladder and kernel
convention are exact for a FIXED pattern; what does not converge is the model's pattern itself.
(3) Compensation cloud read against the SAME-tiling pristine `q0` (the stored static-cell
pattern differs from the tiled cells' by up to 0.24 e per species and had inflated the earlier
`R_eff` 8–11 → 14 Å): z = 0.44 / 0.43 / 0.43 e at 22 / 28 / 34 Å; the net compensation within
4 Å is 19 / 29 / 18 %, within 8 Å 45 / 50 / 39 %, within 10 Å 86 / 82 / 74 %, within 12 Å
99 / 94 / 85 %; `r_gyr` about the vacancy 5.6 / 5.8 / 6.1 Å; the pattern carries a positive
part (+0.44 to +0.47 e) beside the −z (−0.88 to −0.90 e): redistribution, not a monotone
cloud. Well under 80 % inside 4 Å — by this check the gate result is not a code fault. The
cloud does NOT converge with the cell: its 85 % radius moves from ≈ 10 Å (22 Å cell) to
≈ 12 Å (28 Å) to > 12 Å (34 Å), which is why (2) never settles — part of the fill's response
to the missing ion is delocalised over the cell, and the model's `1/L` behaviour is that of
the centred species pattern (53 %) with partial compact compensation. (4) Synthetic sweep
(species pattern + the compensating −Σ as a Gaussian of width w about the vacancy): full-fit
deviation 12 % (r_gyr 2.8 Å) → 21 % (5.2) → 35 % (7.5) → 46 % (9.5) → 50 % (10.5 Å); the model's
49 % sits at r_gyr ≈ 10 Å on that curve — the model's 82–86 % radius, above its |dq0| radius
of gyration (5–6 Å). Verdict: not a code fault (checks 1, 3 and the point-pattern limit of 4
...
**D14 — Route B′ outcome: the selection under both rules, the gates, the winner's checks and the
state of Arm 4 (2026-09-09).** *Campaign.* Twelve runs complete (P3.2b): B′ LR-only s{0, 1, 2, 3,
4, 6} and B′ LR + U s0, s1 on b3 (GPUs 4 and 5, `be3c39b`, four per GPU, 15:13 on 8 Sep → 05:25
on 9 Sep); B′ LR + U s2, s3, s4, s6 on the local A4000 from the snapshot of the same commit
(`~/runs/dscc_src_bp`; s4 / s6 done 00:38 / 00:18, s2 / s3 done 09:30 / 09:20 on 9 Sep — the set
was complete at 09:30, the report at 09:41); the b3 duplicate of s2 (launched 03:52 by the queue runner from an
already-read line) was stopped at 05:25 and removed. Report over all 48 Arm-2+3 runs
(`~/runs/dscc/arm23_report_bp.json`, `arm23_spikes_bp.json`; the v4.4 decision recomputed offline
after `962c79d` gave a B′ winner its Route A Φ = 0 reference). Held-out numbers in the trainer's
convention (per-atom vector RMS) with the per-component value in brackets.

*Selection.* Ranking on seed medians (meV/Å): B′ LR + U 32.8 [18.9], B′ LR-only 32.9 [19.0],
Φ = 0 39.9 [23.0], Route A LR + U 41.3 [23.8], Route A LR-only 41.7 [24.1], Route A full 42.0
[24.2]; λ = 1 43.3 [25.0] leaves on `localisation_stable`; regime A 40.9 not selectable. Per seed:
B′ LR-only 32.9 / 33.0 / 32.8 / 31.2 / 33.1 / 31.8 [19.0 / 19.1 / 18.9 / 18.0 / 19.1 / 18.4], spread
1.9 [1.1]; B′ LR + U 32.2 / 33.0 / 32.5 / 33.1 / 37.5 / 31.7 [18.6 / 19.1 / 18.8 / 19.1 / 21.6 /
18.3]. `tau_noise` (the Φ = 0 seed spread) 1.7, margin `max(tau_phys, tau_noise)` = 3.0. **v4.3
(registered): the equivalent set is {B′ LR + U, B′ LR-only}; the simpler, `B_Bp_lr_only`, is
selected; Φ = 0, Route A LR + U, LR-only and full are beaten beyond the margin.** **v4.4 (post
hoc): the same — `B_Bp_lr_only` selected among the `K_LR` candidates, costed against the Route A
Φ = 0 reference at −7.0 [−4.1] meV/Å: the `K_LR`-carrying candidate costs nothing in forces.**
This reverses D12's selection. B′ entered post hoc — opened by the C10 ruling over its failed
entry gate, on the record — but within the B′ set the pre-registered rule applies unchanged, and
every seed of the winner beats every seed of Φ = 0 (31.2–33.1 against 36.8–41.6). LR + U's
`U_eff` trained to Pb 0.16–0.31 eV (from 1.36), Cl 0.32–0.87, Cs 0.08–0.09: equivalent to "off"
under the rule, "off" the selected value.

*Gates on the winner (`gates()`, C10 readings).* `localisation_stable`: `N_eff` p50 3.50 / 3.40 /
3.26 / 3.28 / 3.49 / 3.49 (Φ = 0 3.27–3.96); root rule on the final model and the last ten epochs
0 % on every seed — no epoch ≥ 1 over the ceiling at all, the initialised-map transient absent as
in Route A LR-only (LR + U: transients end at epochs 7 / 13 / 12 / 18 / 9 / 3, last ten ≤ 2.6 %);
converged 100 %; `f_sr_abs` 0.70–0.72; `s_unsaturated`: `s` 0.557 / 0.489 / 0.486 / 0.501 / 0.499 /
0.625 (LR + U 0.517–0.634) — the pattern trains to about half its initial scale. Far-field gate
(addendum): the B′ LR-only gain over Route A LR-only on the seed medians against the Φ = 0 seed
spread of the same quantity (trainer convention, meV/Å): full RMSE 8.8 vs 1.7, 0–2 Å 10.7 vs
5.9, 2–4 Å 13.2 vs 3.5 (decisive, all beyond noise); 4–6 Å 11.5 vs 2.9, 6–8 Å 8.2 vs 2.6, 4–8 Å
pooled 8.9 vs 2.5 (reported, all beyond noise). LR + U: 8.5 / 8.0 / 12.4 / 11.2 / 6.3 / 7.9 against
the same spreads, beyond noise on every reading. `ladder_rel_err` reported, not gated (below).

*Shells (per component, seed medians, meV/Å; 0–2 / 2–4 / 4–6 / 6–8 / 8–10 / 10–12 / > 12 Å; the
8–10 Å and finer shells exist only for the B′ runs, evaluated after the shell change — the Φ = 0
far shells are D13's diagnostic on s0–s3).* On the report's own five shells (0–2 / 2–4 / 4–6 /
6–8 / > 8 Å, like for like): B′ LR-only 43.4 / 34.7 / 24.0 / 14.9 / 13.5; B′ LR + U 43.9 / 34.5 /
23.7 / 15.3 / 13.8; Φ = 0 48.5 / 39.5 / 28.8 / 18.7 / 18.3; Route A LR-only 49.6 / 42.3 / 30.6 / 19.7 /
18.7 — B′ takes 3.8–5.1 meV/Å per component off every shell including the > 8 Å far field
(4.8). Finer far shells, B′ LR-only from its held-out evaluation (8–10 / 10–12 / > 12 Å) 14.1 /
...
*Loss spikes (logged, not acted on; constant lr 2e-3, final checkpoint).* One per run except
LR-only s3 (none): LR-only s0 epoch 20 (66 × the median; held 34.3 → 55.1 at the next evaluation,
recovered to 32.9), s1 55 (3.8 ×), s2 29 (7.8 ×; 33.0 → 47.5), s4 32 (12 ×; 31.2 → 38.1), s6 59
(3.5 ×); LR + U s0 36, s1 33, s2 54, s3 40, s4 38 (9.5 ×; 34.6 → 43.7 — s4's 37.5 is the one B′
outlier), s6 48. The ±10 meV/Å final-checkpoint noise of D12 remains in every reading. **The §9
robustness pass is not run: the user's instruction of 9 Sep is no retraining.**

*Checks on the winner (9 Sep).* (1) Forces: the trained `B_Bp_lr_only_s0` on a real held-out
79-atom charged frame (index 1622; 12 SCF iterations; `s` 0.557), inference pair route against
central differences of the energy (h = 2e-3 Å) on five components of atoms 12, 30 and 55:
|ΔF| ≤ 2.2e-5 eV/Å on forces up to 0.74 eV/Å; pair route against the cotangent route on the whole
frame 1.6e-15 (`scratchpad/bp_fd_check.py`). (2) Ladder gate on the six winners' `H0`
(`arm23_bp_gates.json`; a model property, reported): the `q0` slope 40 / 34 / 37 / 39 / 35 / 29 %
off Madelung (the species-centred pattern 51 % on every seed); precondition 96–99 %, separation
0.75–0.94 eV. (3) `K_LR` ladder decomposition on s0 (`ladder_bp_lr_only_s0_decomp.json`;
static-cell tilings, CPU): `E(+1) − E(0)` 7.960 / 8.034 / 8.079 eV on the same-shape 79 / 159 /
639-atom cells (8.118 on 2×2×1) fits a `1/L` coefficient of −3.30 eV·Å = 67 % of the expected
−4.90 (Route A LR-only: 24 %); the head's `½ dqᵀ Γ dq` fits −4.40 (90 % of Madelung), the host
term `dqᵀ W` −0.84, the band term +1.95 (Route A: −4.35 / — / +3.19); `N_eff` 3.6–3.8 on every
cell. The kernel carries the image term; the static well adds a `1/L` piece of its own (its
compensation is size-dependent, D13) and the band term's Γ-point error is smaller than in Route
A. Diagnostic; `E_SF` stays out of cross-size energy claims (addendum). (4) 2× benchmark (P4.3,
idle A4000, warm-started charged trajectory, base included; `benchmark_bp_{79,159}.json`): 79
atoms base 43.5 ms, model 305 ms, ratio median 7.03, p95 7.43 (100 frames); 159 atoms 73.7 /
632 ms, median 8.59, p95 13.7 (16 frames); SCF median 4 iterations; the 79-atom run repeated on
the idle machine (the first overlapped the CPU ladder jobs): 43.6 / 307 ms, median 7.04, p95 7.72,
unchanged. **Fails the registered 2×
criterion by a factor of 3.5–4** (the Φ = 0 winner: 1.98 / 2.36 and 1.92 / 5.66): the coupled
head costs about six base forwards (SCF fills, the reference fill of `H0` for `q0`, the Ewald
matrices at tol 1e-16, the `Γ_LR` pair route) — not profiled; Arm 4 decision (5) and §8 read
against this number.

*Consequences.* (a) Production status is provisional: plan §7 selects D-SCC as production only if
decision (1)–(6) hold against the matched-kernel comparator (P3.3). The matched F-SCC (three
seeds, λ and U learned, GPU 7) stood at epoch 30 of 60 at 10:40 on 9 Sep (≈ 40 min per epoch,
finals ≈ 07:00 on 10 Sep) and read 29.8 / 33.2 / 30.1 [17.2 / 19.2 / 17.4] at epoch 29 — at or
below the B′ finals at half training. (b) Decision (4), the sparse-path agreement, cannot be read
for B′ as it stands: `q0` is the full Mulliken fill of `H0`, which the frontier solver does not
produce (P4.2). (c) The double-screening and `E_SF` caveats stand (§3.7). (d) D12's "electrostatics
adds nothing resolvable" was a statement about the same-carrier terms; the host term reversed it,
as the C10 ruling anticipated.

*Scientific record.* Same-carrier electrostatics (Route A) added nothing resolvable at 79 atoms;
the host pattern (Route B′: `K_LR` with the static well at about half its initial scale) takes
4–5 meV/Å per component off every held-out shell including the > 8 Å far field, closes most of
...
59.1; 6.6–6.8 16.2 / 23.3; 6.8–7.0 27.7 / 15.0 ([2] unless stated). Three bands: (i) d < 5.2 Å — the head's residual is EQUAL at the two sizes
(159 in-sample 72.7 / 47.0 against 71.0 / 49.7 at 79; one 159 frame per bin, trained on, and 7 of
256 frames at 79) — the one place a capacity signal survives, unresolved; (ii) d 5.2–6.4 Å, the
bulk (≈ 85 % of the 79-atom frames) — the base's residual is the same at both sizes (150–270, a
property of the local geometry) while the head's is 43–53 at 79 against 8–18 held-out and 10–35
in-sample at 159, i.e. 1.2–6 × smaller on the same geometry; (iii) d ≥ 6.4 Å — both diverge, the
base 120–130 at 79 against 20–60 at 159 and the head 50–88 against 16–31, so the 79-atom cell adds
an error source at large d that fades at 159. Label check: the
...
clauses, and the verdict by the letter is **mixed**. Weight of the evidence: in the bulk of the
matched-d range (5.2–6.4 Å) the base's flanking-Pb residual is size-independent while the head's
is 1.2–6 × smaller at 159 on the same geometry, and at d ≥ 6.4 Å the 79-atom cell adds an error
source to base and head alike that fades at 159; only the d < 5.2 Å corner (one 159 frame per
bin) shows equal head residuals at the two sizes; the base's near-field error is systematic (it predicts the neutral dimer
...
A `b_i(h_i)` capacity arm read at 79 atoms would chase that; not indicated on this evidence for
the bulk, with the d < 5.2 Å corner open. If it runs, its metric must be the paired 79 / 159
flanking-Pb residual at matched d (the leak readout) with the d < 5.2 Å bins reported separately,
not the 79-atom excess. Caveats: the 159 held-out set is 4 frames (8 flanking Pb, 31
...
| P3.2b | Route B′ arms (C10, 2026-09-08): regime B × route B′ × coupling {LR-only; LR + U; full; λ = 1} × 6 seeds = 24 runs, plus the regime-A B′ ablation (6); Φ = 0 reference = the Route A Φ = 0 runs; same protocol (60 epochs, lr 2e-3, final checkpoint), each seed from its Arm-1 `H0`; pair force route extended to B′; selection gates as registered with the C10 readings (far-field 4–8 Å pooled gain over the same-coupling Route A arm beyond the Φ = 0 seed spread; `s` in [0.1, 1.9]; the ladder gate awaiting C10 (e)) | **launched 2026-09-08 15:13 (b3 clock) on `be3c39b`**, GPUs 4 and 5 at four per GPU, LR-only and LR + U first, the ablation last (cut candidate); **trimmed by the C10 addendum (15:41) to LR-only and LR + U: 12 runs, wave 1 (8) + 4, ≈ 26 h**; full / λ = 1 / the ablation not run (the far field is periodicity-suppressed at these sizes); D13 for the entry-gate re-check, memory, smoke. **Interim (2026-09-09 05:30): the six B′ LR-only runs are complete — held-out charged force RMSE 18.0–19.1 meV/Å per component on every seed (median 19.0, seed spread 0.5; trainer convention 32.9) against the Route A LR-only finals 21.5–32.1 (median 24.1; 41.7) and the Φ = 0 references 21.2–24.0 (median 23.0; 39.9): 7.0 meV/Å (trainer convention) below Φ = 0, more than twice the registered 3.0 margin, on every seed; every shell improves, including the far field (> 8 Å: 13–14 vs 18–19 for Route A); root rule 0 % on the last ten epochs of every seed; one loss spike (s4); pattern scale `s` 0.49–0.63. LR + U: s4 21.6, s6 18.3 final (local); s0/s1 at epochs 57/59 on b3 reading 19.4–19.6 at epoch 54; s2/s3 local at epoch ~28. The b3 queue runner had already read the s2 line before the 00:41 truncation and launched a duplicate at 03:52; it was stopped at 05:25 and its directory removed (the local s2 run is the one of record). Selection reading deferred to the full set (D14).** | driver / watcher / stop scripts in the scratchpad **Complete 2026-09-09 09:30 (12 runs; b3 runs by 05:24, local s2 / s3 at 09:20 / 09:30; D14): both rules select `B_Bp_lr_only`; LR + U equivalent (32.8 vs 32.9); B′ gates pass; report `~/runs/dscc/arm23_report_bp.json`.** |
| P3.3 | Arm 4: matched-kernel and full-kernel F-SCC comparators; decision (1)–(6) | **matched-kernel comparator launched 2026-09-08 15:13 (b3 clock) on `be3c39b`: `dscc_arm4_matched_s{0,1,2}` on GPU 7 at three per GPU (C10: three seeds; λ, U learned; ≈ 35–45 h per run, D13); the full-kernel comparator and the frozen-coupling variant not queued; decision (1)–(6) reads against the D14 selection `B_Bp_lr_only` (32.9 [19.0 per component]) once the runs finish; interim 2026-09-09 10:40: epoch 30 of 60 on all three seeds, held-out 29.8 / 33.2 / 30.1 [17.2 / 19.2 / 17.4] at epoch 29, finals ≈ 07:00 on 10 Sep; decision (4) cannot be read for B′ (no sparse B′ path, P4.2); decision (5) reads the D14 benchmark (7.0 / 8.6 ×)** — was: todo after the Arm-2+3 selection; F-SCC training path smoke-tested (`--fscc matched`); its kernel force routed through the pair derivatives too (commit `8a5ca51`, `TestFsccPairForcePath`), so Arm 4 trains at the same memory as Arm 2+3. **Engineering item before Arm 4 (v4.3 §8):** base features and head features in one graph with a single backward (no early-block recomputation), then re-measure the 2× benchmark with ≥ 100 frames per size on the Φ-on model |
...
- [ ] **End-of-programme robustness pass on the selected model only (v4.3):** LR decay 2e-3 → 2e-4 over the last 20 epochs, spike detection with rollback, epoch-averaged final checkpoint; retrain the selected configuration and its Φ = 0 reference under that protocol and report whether any selection decision changes; no decision made before the pass is revisited unless it does. **After the Route B′ campaign (D14, 2026-09-09) the selected configuration is `B_Bp_lr_only` with the Route A Φ = 0 runs as its reference; the pass is NOT run — the user's instruction of 9 Sep is no retraining. If it is ever run, the decay form, the spike criterion and the averaging window are not fixed by the v4.3 text and must be registered first.**
