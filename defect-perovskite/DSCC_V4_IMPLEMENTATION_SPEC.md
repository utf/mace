...
2026-09-06 evening) `DSCC_PLAN_V4_3_AMENDMENT.md` (2026-09-07: C8 ruling — precedence of the C5 gate over route C's stop; Arm 2+3 protocol = the Arm-1 protocol; §9 deferred robustness pass; benchmark engineering item), `DSCC_PLAN_V4_5_AMENDMENT.md` (2026-09-07: SCF warm starts during training, C9) and `DSCC_PLAN_V4_2_AMENDMENT.md` (C5–C7 rulings: Route B′,
...
shape-slope error (median over the four seeds with a fold-1/3 shape residual; seeds 0 and 2
have none), `N_eff` p50 median (seed spread), the worst-epoch (≥ 1) and last-epoch
...
LR + U 84 / 72 / 52 / 33 / 32, full 93 / 74 / 54 / 34 / 32, λ = 1 90 / 74 / 54 / 36 / 33 — the
coupled arms lose in the near shells and match Φ = 0 beyond 6 Å. The base alone reads
...
`localisation_stable`; `beaten_beyond_margin` empty in every reading. Φ = 0 is selected in
all four as the simplest equivalent configuration. (`tau_noise` is the seed standard
...
run's final-epoch check is 0/39. The failures are the transient from the multi-valued
...
in the full arm, with no force gain at any shell and a loss inside 4 Å; the bound state is
