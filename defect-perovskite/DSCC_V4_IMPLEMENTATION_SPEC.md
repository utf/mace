...
| P3.2 | Arm 2+3 (v4.1 layout, v4.3 protocol): regime B — route A × coupling {Φ = 0; LR-only; LR+U; full; λ = 1 fixed} = 5 configurations × 6 seeds {0, 1, 2, 3, 4, 6} = 30 runs, plus the regime-A ablation (full coupling, route A, 6 seeds), excluded from selection; Route B′ closed by its ladder gate; thresholds registered; forecasts §2.10 checked | **launched 2026-09-07 08:50** (v4.3 ruling; nine seeds queued, cut to six on the user's instruction five minutes later by truncating the b3 queue in place — no run of seeds 7/9/10 had started): b3 queue on GPUs 4/5/7, three per GPU (`~/runs/arm23_queue_b3.log`, 34 runs), two runs local from a snapshot after the entry-gate jobs finish; 36 runs × 60 epochs |
