...
exec 9>"${TMPDIR:-/tmp}/mace_exp1b.lock"
flock -n 9 || { echo "another run_experiment1b.sh is running" >&2; exit 1; }

for seed in ${SEEDS:-1 2 3}; do
    name="e1b_logit_s${seed}"
    ...
    DEFECT_LOGIT_SEED_GAMMA="${GAMMA:-1.5}" \
    "${HERE}/train_defect_model.sh" > "$HOME/runs/$name.log" 2>&1 &
done
wait
echo "experiment 1b (logit seed) complete"
