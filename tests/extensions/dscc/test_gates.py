...
def _model(route_b, tol_q):
    m = MACEDSCC(_base(0), r_cut=6.0, directional=True, hidden=16, coupling=True, route_b=route_b,
                 ...
                 scf=ScfOptions(tol_q=tol_q, tol_E=max(tol_q, 1e-11), continuation_steps=0))   # see test_model._coupled
    with torch.no_grad():
...
def _energy(m, atoms, warm=None):
    return float(m(_batch([atoms]), compute_force=False, warm_start=warm)["energy"])
...
def test_force_step_ladder_and_scf_tolerance(route_b):
    ...
    for tol_q in (1e-6, 1e-10):
        ...
        ref = m(_batch([VACP]), compute_force=True)
        analytic = float(ref["forces"][atom, comp])
        warm = [ref["dq"].detach()]                     # one branch for every displaced solve
        errs = []
        for h in (1e-2, 1e-3, 1e-4):
            plus, minus = VACP.copy(), VACP.copy()
            plus.positions[atom, comp] += h
            minus.positions[atom, comp] -= h
            fd = -(_energy(m, plus, warm) - _energy(m, minus, warm)) / (2 * h)
...
def test_all_six_strain_components():
    ...
    for i, j in ((0, 0), (1, 1), (2, 2), (0, 1), (0, 2), (1, 2)):
        ...
        for sign in (1.0, -1.0):
            ...
            e.append(_energy(m, strained, warm))
